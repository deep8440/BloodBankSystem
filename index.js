...
sgMail.setApiKey(
  "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"
);
...
myApp.post(
  ...
  function (req, res) {
    ...
    BloodBank.findOne({ email: email, password: password }).exec(function (
      err,
      bloodBank
    ) {
      if (bloodBank) {
        // would be true if user is found in admin user
        // save in session
...
myApp.get("/logoutuser", function (req, res) {
  // clear session
  req.session.userid = '';
  req.session.useremail_user = '';
  req.session.username_user = '';
  req.session.loggedId_user = true;
...
myApp.post("/bloodbankSignup", [], function (req, res) {
  //fetch all the form fields
...
myApp.get("/profile", function (req, res) {
  if (req.session.loggedId) {
    BloodBank.findOne({ email: req.session.bloodbankemail }).exec(function (
      err,
      bloodbank
    ) {
      var userData = {
...
myApp.post("/profileSubmit", [], function (req, res) {
  //fetch all the form fields
  var name = req.body.name;
  ...
  //find in database if it exits
  BloodBank.findOne({ email: req.session.username }).exec(function (
    err,
    bloodbank
  ) {
    (bloodbank.name = name),
...
myApp.post("/postalcodesearch", [], function (req, res) {
  ...
  //find in database if it exits
  BloodBank.find({ postalCode: postalcode.toLowerCase() }).exec(function (
    err,
    bloodbanks
  ) {
    if (bloodbanks) {
...
myApp.post("/select/bookappointment", [], function (req, res) {
  ...
  console.log(pageData.bloodbankaddress1);
  console.log(pageData.bloodbankcity);
  let message =
    "Hello " +
    username +
    "," +
    "Your appointment to donate blood is booked on " +
    bookingdate +
